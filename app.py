...
@app.route('/foretellers', methods = ['GET'])
def foretellers():
    my_foretellers = ['Scarlett', 'Aydee', 'Patrick', 'Armani', 'Jalen']
                                                # var_name in Jinja = var name in python
    return render_template('foretellers.html.j2', foretellers = my_foretellers)
...
def horoscopes():
    if request.method == 'POST':
        ...
        if not response.ok:
            ...
            return render_template('horoscopes.html.j2', error=error_string)
        ...
        return render_template('horoscopes.html.j2', racers=new_data)
    return render_template('horoscopes.html.j2')